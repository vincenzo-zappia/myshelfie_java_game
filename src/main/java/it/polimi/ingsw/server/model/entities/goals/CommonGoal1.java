        ...
        try {
            for(int i = 0; i < 5 ; i++){  //search from row 0 to row-1
                for(int j = 0; j < 4; j++){ //search from column 0 to column-1
                    if(!bs.getCell(i,j).isCellEmpty() && !bs.getCell(i,j+1).isCellEmpty() && !bs.getCell(i+1,j).isCellEmpty() && !bs.getCell(i+1,j+1).isCellEmpty()){  //check the cell if it is empty or not
